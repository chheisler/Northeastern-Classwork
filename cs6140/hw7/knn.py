...
def polynomial(datum, data):
	return (data * datum).sum(axis=FEATURE_AXIS) ** 2
...
def digits(k, fn):
	...
	# test the data
	train_error = test(train_data, train_labels, train_data, train_labels, k, fn)
	test_error = test(test_data, test_labels, train_data, train_labels, k, fn)
