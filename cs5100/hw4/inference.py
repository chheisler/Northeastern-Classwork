...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        distribs = {}
        for prev_p in self.legalPositions:
            self.setGhostPosition(gameState, prev_p)
            distribs[prev_p] = self.getPositionDistribution(gameState)
        for p in self.legalPositions:
            allPossible[p] = sum([
                self.beliefs[prev_p] * (distribs[prev_p].get(p) or 0.0)
                for prev_p in self.legalPositions
            ])
            # prob = 0.0
            # for prev_p in self.legalPositions:
                # self.setGhostPosition(gameState, prev_p)
                # pos_prob = self.getPositionDistribution(gameState).get(p) or 0.0
                # prob += self.beliefs[prev_p] * pos_prob
            # allPossible[p] = prob
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        self.particles = [
            self.legalPositions[x % len(self.legalPositions)]
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        if noisyDistance is None:
            particles = [self.getJailPosition()] * self.numParticles
        else:
            distrib = util.Counter()
            for p in self.particles:
                true_dist = util.manhattanDistance(p, pacmanPosition)
                distrib[p] += emissionModel[true_dist]
            if sum(weight for p, weight in distrib.iteritems()) == 0:
                particles = [
                    random.choice(self.legalPositions)
                    for x in range(self.numParticles)
                ]
            else:
                particles = [
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            ...
            distrib = self.getPositionDistribution(gameState)
            particles.append(util.sample(distrib))
        self.particles = particles
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = particles[:self.numParticles]
    ...
    def observeState(self, gameState):
        ...
        for p in self.particles:
            ...
            for i, pos in enumerate(p):
                ...
                if noisyDistance is None:
                    new_p.append(self.getJailPosition(i))
                else:
                    true_dist = util.manhattanDistance(pos, pacmanPosition)
                    weight *= emissionModels[i][true_dist]
                    new_p.append(pos)
            distrib[tuple(new_p)] += weight
        if sum([weight for p, weight in distrib.iteritems()]) == 0:
            particles = [
                tuple(
                    self.getJailPosition(i)
                    if dist is None
                    else random.choice(self.legalPositions)
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                distrib = getPositionDistributionForGhost(gameState, i, self.ghostAgents[i])
