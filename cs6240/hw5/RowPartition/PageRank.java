...
public class PageRank {
	...
	private static int ABSOLUTE_INDEX = 3;
	...
	throws Exception, IOException, InterruptedException, ClassNotFoundException {
		Job job = makeJob(config, "ITERATE");
		...
		// add rank vector to distributed cache
		String fullOutput;
		if (absolute)
			fullOutput = System.getProperty("user.dir") + "/" + output + "/";
		else fullOutput = output + "/";
		
		for (int i = 0; i < numReducers; i++) {
			String filename = fullOutput + rankDir + "/" + partFormat.format(i);
